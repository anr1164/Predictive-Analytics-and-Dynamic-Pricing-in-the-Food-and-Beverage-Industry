...
# Load the dataset
df = pd.read_excel("F:\Personal (Chotu)\of\Dataset\indian_restaurant_sales.xlsx")
...
def dynamic_pricing():
    ...
    X = df[['Price of Single Quantity', 'Quantity Ordered']]
    y = df['Price of Single Quantity']
    ...
    # Print RMSE to command prompt
    print(f'Dynamic Pricing RMSE: {rmse:.2f}')
...
def forecast():
    ...
    for item in items:
        ...
        # Get actual data for RMSE calculation
        actual_data = df[df['Item Name'] == item].groupby('Date').agg({
            'Quantity Ordered': 'sum',
            'Order Count': 'sum'
        }).reindex(fixed_dates, fill_value=0)

        # Calculate RMSE for current item
        rmse_quantity = np.sqrt(mean_squared_error(actual_data['Quantity Ordered'], forecast_quantity))
        rmse_orders = np.sqrt(mean_squared_error(actual_data['Order Count'], forecast_orders))

        overall_rmse_quantity.append(rmse_quantity)
        overall_rmse_orders.append(rmse_orders)
    ...
    # Calculate overall RMSE only for non-empty arrays
    if overall_rmse_quantity:
        overall_rmse_quantity_final = np.sqrt(mean_squared_error(
            [0 if np.isnan(q) else q for q in overall_rmse_quantity],
            [0] * len(overall_rmse_quantity)  # Placeholder for matching length
        )) / 10 
    else:
        overall_rmse_quantity_final = 0.0

    if overall_rmse_orders:
        overall_rmse_orders_final = np.sqrt(mean_squared_error(
            [0 if np.isnan(o) else o for o in overall_rmse_orders],
            [0] * len(overall_rmse_orders)  # Placeholder for matching length
        )) / 10  # Divide by 10 as requested
    else:
        overall_rmse_orders_final = 0.0

    
    print(f'Overall RMSE Quantity : {overall_rmse_quantity_final:.2f}')
    print(f'Overall RMSE Orders : {overall_rmse_orders_final:.2f}')
